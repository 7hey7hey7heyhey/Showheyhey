...
import os 
os.getcwd() # 現在のディレクトリを表示
os.chdir("C:\\Users\\u00180118\\Desktop\\python code") # ディレクトリの変更
...
for po in path.iterdir(): # •ディレクトリ内のファイル・サブディレクトリ一覧を取得 iterdir
    ...
    if not po.is_dir(): # フォルダでない場合(つまり、何らかのファイルの場合（例えば、エクセルファイルとかWordファイルのとき）)に表示する。
        file_list1.append(po) # po をリストに追加する（poとは何らかのファイル）。
        # print(type(po.is_dir()))
        # print(po)

print(file_list1) # ディレクトリ内のファイルを確認する。
# file_list1[0]
# file_list1[1]
...
#####
kensaku = 'このショートカットに' # 検索したい言葉をここに入れる。
